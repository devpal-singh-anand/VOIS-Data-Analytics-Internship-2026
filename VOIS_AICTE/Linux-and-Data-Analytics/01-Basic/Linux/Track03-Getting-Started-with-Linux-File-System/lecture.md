...
* becoming familiar with the **Linux file system**.
* understanding **file attributes and directory management commands**.
...
* The **root `/` directory**.
* Linux file-system types such as **EXT3, EXT4, BTRFS, and XFS**.
* The **Linux Virtual File System (VFS)**.
* Linux file-system features such as:
...
* Moving and renaming files and directories using `mv`.
* Removing files using `rm`.
* Copying files and directories using `cp`.
* Creating and editing files using **Nano** and **Vim**.
* Viewing file contents using:

  * `cat`
  * `less`
  * `head`
  * `tail`
* Displaying and writing text using `echo`.
* Using `sudo` to execute commands with elevated privileges.
* Managing files and directories efficiently from the Linux command line.

The course provides the foundation required to work comfortably with the **Linux file system and basic file-management commands**.
